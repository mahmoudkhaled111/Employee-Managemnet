...
namespace EmployeeMangement.Controllers
{
    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> ManageUserClaims(UserClaimsViewModel model)
        {
            ...
            if(user == null)
            {
                ViewBag.ErrorMessage = $"the user with id = {model.userId} cannot found";
                return View("NotFound");
            }
        ...
        public async Task<IActionResult> ManageUserRoles(List<UserRolesViewModel> model, string userId)
        {
            ...
            if (!result.Succeeded)
            {
                ModelState.AddModelError(string.Empty, "Can remove user existing roles");
                return View(model);
            }
            ...
            if (!result.Succeeded)
            {
                ModelState.AddModelError("", "Cannot add selected roles to uesr");
            }
            ...
            public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            else
            {
                ...
                catch (DbUpdateException ex)
                {
                    ...
                    ViewBag.ErrorMessage = $"{role.Name} role cannot be deleted are there users in this role. " +
                        $"If you want to delete this role,firstly romove the user " +
                        $"form the role and try to delete role again";
                    return View("Error");
                }
            }
        }
        ...
        public async Task<IActionResult> EditUserInRole(List<UserRoleViewModel> model, string roleId)
        {
            ...
            for (int i = 0; i < model.Count; i++)
            {
                ...
                if (user == null)
                {
                    ViewBag.ErrorMessage = $"user id IS NULL";
                    return View("NotFound");
                }
                else
                {
